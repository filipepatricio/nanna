...
B64_ENV=$(echo -n "env=integration_prod" | base64 -w 0)
B64_ACCESS_TOKEN=$(echo -n "accessToken=$ACCESS_TOKEN" | base64 -w 0)

flutter build apk --debug --flavor prod --dart-define=env=integration_prod --dart-define=accessToken=$ACCESS_TOKEN
