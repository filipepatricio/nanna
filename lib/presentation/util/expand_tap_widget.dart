...
class _ExpandTapRenderBox extends RenderBox with RenderObjectWithChildMixin<RenderBox> {
  ...
  void debugPaintExpandArea(PaintingContext context, Offset offset) {
    ...
    final childParentData = child!.parentData! as BoxParentData;
    final paintOffset = childParentData.offset + offset - tapPadding.topLeft;
  ...
  void handleEvent(PointerEvent event, BoxHitTestEntry entry) {
    if (event is PointerDownEvent) {
      final member = _TmpGestureArenaMember(onTap: onTap);
  ...
  bool hitTest(BoxHitTestResult result, {Offset? position}) {
    final expandRect = Rect.fromLTWH(
      0 - tapPadding.left,
      0 - tapPadding.top,
    ...
    if (expandRect.contains(position!)) {
      final hitTarget = hitTestChildren(result, position: position) || hitTestSelf(position);
      if (hitTarget) {
